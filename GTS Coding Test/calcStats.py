...
outputFile = NamedTemporaryFile('w', newline='', delete=False) # can't edit in place effectively, so write to temp file as we go
...
with open(inputFile) as csvFile:
    ...
    for row in reader: # each row is a trade
        ...
        if row[2] != 'TRADE': # writes first row with titles
            ...
            writer.writerow(row)
    outputFile.close()
    shutil.move(outputFile.name,inputFile)
    ...
    print('')
    print('Average Trade Size:', statistics.mean(totalSharesTraded))
    print('Median Trade Size:', statistics.median(totalSharesTraded))
