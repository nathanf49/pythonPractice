# one = 2 segments - unique
# two = 5 segments
# three = 6 segments
# four = 4 segments - unique
# five = 5 segments
# six = 6 segments
# seven = 3 segments - unique
# eight = 7 segments - unique
# nine = 6 segments
...
def sevenSegmentDecode2(segments): # TODO letters are all mixed up and do not actually mean what directions say they do
    total = 0
    for line in segments:
        lineNumber = ''
        for s in line:
            if len(s) == 7:  # unique length
                lineNumber += '8'
            elif len(s) == 3:  # unique length
                lineNumber += '7'
            elif len(s) == 4:  # unique length
                lineNumber += '4'
            elif len(s) == 2:  # unique length
                lineNumber += '1'
            elif ('c' in s) and ('d' in s) and ('f' in s) and ('b' in s) and ('e' in s) and (len(s) == 5):
                lineNumber += '5'
            elif ('g' in s) and ('c' in s) and ('d' in s) and ('f' in s) and ('a' in s) and (len(s) == 5):
                lineNumber += '2'
            elif ('f' in s) and ('b' in s) and ('c' in s) and ('a' in s) and ('d' in s) and (len(s) == 5):
                lineNumber += '3'
            elif ('c' in s) and ('a' in s) and ('g' in s) and ('e' in s) and ('d' in s) and ('b' in s) and (len(s) == 6):
                lineNumber += '0'
            elif ('c' in s) and ('e' in s) and ('f' in s) and ('a' in s) and ('b' in s) and ('d' in s) and (len(s) == 6):
                lineNumber += '9'
            elif ('c' in s) and ('d' in s) and ('f' in s) and ('g' in s) and ('e' in s) and ('b' in s) and (len(s) == 6):
                lineNumber += '6'
            else:
...
if __name__ == '__main__':
    with open('test_sevenSegment.txt') as file:
        inputData = file.readlines()
        file.close()
    for f in range(len(inputData)):
        inputData[f] = inputData[f].split('|')
        inputData[f] = inputData[f][1]
        inputData[f] = inputData[f].split()

    sevenSegmentDecode2(inputData) # 200 is too low
...
The problem is that the signals which control the segments have been mixed up on each display. The submarine is still t
rying to display numbers by producing output on signal wires a through g, but those wires are connected to segments 
...
entry, the same wire/segment connections are used (but you don't know what the connections actually are). The unique s
ignal patterns correspond to the ten different ways the submarine tries to render a digit using the current wire/segment
