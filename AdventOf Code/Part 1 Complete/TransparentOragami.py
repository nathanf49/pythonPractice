def transparentOragami(points,folds):
    """Calculates how many points are visible after folds.
    Some points can't be seen if they are on top of another point"""

    for fold in folds:
        for p in range(len(points)):
            if fold[0] == 'y':
                if points[p][1] > fold[1]:
                    points[p][1] -= (2 * (abs(points[p][1] - fold[1])))  # distance from y is the same, now above

            else: # fold[0] is x
                if points[p][0] > fold[1]:
                    points[p][0] -= (2 * (abs(points[p][0] - fold[1])))  # same distance from x, now to the right
...
if __name__ == '__main__':
    """ import and parse data before passing to transparentOragami function"""
    file = 'test_TranparentOragami.txt'
    with open(file) as f:
        data = f.readlines()
        f.close()
    folds = []
    ...
    transparentOragami(points,folds)
...
instruction is fold along y=7, which designates the line formed by all of the positions where y is 7 (marked with -):
